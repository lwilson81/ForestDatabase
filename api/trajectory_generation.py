...
def print_dance(robots, final_time):
    ...
    [trajectory, velocity] = traj.generate_trajectory(
        dances, dances_t, final_time)

    for robot in range(NUM_ROBOTS):
        robot_loc = 7 * (robot)
        for i in range(7):
            trajectory[i + robot_loc, :] = trajectory[i +
                                                      robot_loc, :] + INIT_POS[i]
    ...
    df.to_csv("/Users/jocekav/Documents/GitHub/Forest/flock.csv",
